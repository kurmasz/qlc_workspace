...
class HTTPSocket: 
    ...
    def send_text_line(self, message):
        ...
        if (self.verbose):
            ...
            sys.stderr.flush()
 # Check if message is already bytes, don't encode if it is
        if isinstance(message, str):
            self.socket.sendall(message.encode('utf-8') + CR_LF)
        else:  # message is already bytes (e.g., for binary data)
            self.socket.sendall(message + CR_LF)
